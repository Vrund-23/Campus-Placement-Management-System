...
interface TPCResult extends TPCRow {
...
export default function ManageTPCPage() {
  ...
  const [existingTPCs, setExistingTPCs] = useState<ExistingTPC[]>([]);
  const [uploadedRows, setUploadedRows] = useState<TPCRow[]>([]);
  const [results, setResults] = useState<TPCResult[] | null>(null);
  ...
  const fetchExistingTPCs = async () => {
    try {
      setLoading(true);
      const data = await api.get(`/tpc-management?academicYear=${selectedYear}`);
      setExistingTPCs(data);
    } catch (err: any) {
      console.error('Failed to fetch TPCs:', err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      const rows: TPCRow[] = json.map((row: any) => ({
      ...
      })).filter((r: TPCRow) => r.name && r.email);
      ...
      toast({ title: `${rows.length} TPC(s) loaded`, description: 'Review the list below and click "Create & Send Emails".' });
    };
  ...
  const handleSyncExcel = async () => {
    try {
      setSyncing(true);
      setResults(null);
      const res = await api.post('/tpc-management/sync', { academicYear: selectedYear });
      ...
      toast({ 
        title: 'Sync Complete', 
        description: `Successfully created ${created} and updated ${updated} TPCs.` 
      });
      fetchExistingTPCs();
    } catch (err: any) {
      toast({ 
  ...
  const handleUploadAndSync = async () => {
    ...
    try {
      ...
      // 1. Upload
      await api.postFormData(`/tpc-management/upload?academicYear=${selectedYear}`, formData);
  ...
  const handleSendAll = async () => {
    // Standard bulk invite from PREVIEW data (legacy)
  ...
  const handleDeleteTPC = async (tpc: ExistingTPC) => {
    try {
      setDeletingId(tpc.user_id);
      await api.delete(`/tpc-management/${tpc.user_id}`);
      toast({
        title: 'TPC removed',
        description: `${tpc.name} has been removed from the system.`,
      });
      setExistingTPCs(prev => prev.filter(t => t.user_id !== tpc.user_id));
      setConfirmDeleteId(null);
    } catch (err: any) {
      toast({
        title: 'Failed to remove TPC',
  ...
  const downloadTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'TPCs');
    XLSX.writeFile(wb, 'TPC_Template.xlsx');
  };
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              <Users className="h-6 w-6 text-primary" /> Manage TPCs
            </h1>
            <p className="text-muted-foreground">Upload an Excel file with TPC details, review, and send login credentials.</p>
          </div>
          ...
          <CardContent className="pt-6">
            {uploadedRows.length === 0 && !results ? (
              ...
              >
                ...
                <h3 className="text-lg font-semibold mb-1">Upload TPC Excel File</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between bg-muted/50 rounded-lg px-4 py-2">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <Badge variant="secondary">{uploadedRows.length} TPCs</Badge>
                  </div>
                ...
                <div className="flex justify-end gap-3 pt-2">
                  {results ? (
                    /* After creation: show "Add More TPCs" button */
                    <Button variant="outline" className="gap-2" onClick={handleAddMore}>
                      <Plus className="h-4 w-4" /> Add More TPCs
                    </Button>
                  ) : (
                    ...
                    <>
                      ...
                      <Button className="gap-2 bg-emerald-600 hover:bg-emerald-700 shadow-lg text-white font-bold" onClick={handleUploadAndSync} disabled={uploading}>
                        {uploading ? (
                          ...
                          <><Upload className="h-4 w-4" /> Upload & Sync TPC List</>
                        )}
                      </Button>
                    </>
                  )}
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle>Active TPCs</CardTitle>
                <CardDescription>{existingTPCs.length} coordinator{existingTPCs.length !== 1 ? 's' : ''} registered</CardDescription>
              </div>
              {existingTPCs.length > 0 && !isPastYear && (
          ...
          <CardContent>
            {loading ? (
            ...
            ) : existingTPCs.length === 0 ? (
              <div className="text-center py-12 text-muted-foreground">
                <Users className="h-12 w-12 mx-auto mb-3 opacity-20" />
                <p className="font-medium">No TPCs added yet</p>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-border/50">
                    {existingTPCs.map((tpc) => (
                      <tr key={tpc.user_id} className="hover:bg-muted/30 transition-colors group">
                        <td className="py-3 pr-4">
                          <div className="flex items-center gap-3">
                            <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xs">
                              {tpc.name?.charAt(0)?.toUpperCase() || '?'}
                            </div>
                            <span className="font-medium">{tpc.name}</span>
                          </div>
                        </td>
                        <td className="py-3 pr-4 text-muted-foreground">{tpc.email}</td>
                        <td className="py-3 pr-4"><Badge variant="secondary">{tpc.department_name}</Badge></td>
                        <td className="py-3 pr-4 font-mono text-xs text-muted-foreground">{tpc.college_id || '—'}</td>
                        <td className="py-3 text-right">
                          {confirmDeleteId === tpc.user_id ? (
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <Button
                                ...
                                disabled={deletingId === tpc.user_id}
                                onClick={() => handleDeleteTPC(tpc)}
                              >
                                {deletingId === tpc.user_id ? (
                            ...
                            <Button
                              ...
                              onClick={() => setConfirmDeleteId(tpc.user_id)}
                            >
